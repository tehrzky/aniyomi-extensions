...
class Kajzu : ConfigurableAnimeSource, ParsedAnimeHttpSource() {
    ...
    // ============================== Episodes ==============================
    override fun episodeListSelector(): String = "ul.list-episode-item-2.all-episode li a, .episode-list li a, .episodes a"

    override fun episodeFromElement(element: Element): SEpisode {
        return SEpisode.create().apply {
            setUrlWithoutDomain(element.attr("href"))
            val titleElement = element.selectFirst("h3.title, .title, a")
            ...
            val type = element.selectFirst("span.type, .language")?.text() ?: "SUB"
    ...
    override fun videoListParse(response: Response): List<Video> {
        ...
        if (serverLinks.isEmpty()) {
            ...
            val method2Elements = document.select(".server-list li, ul.list-server-items li, .anime_muti_link li, .player-list li")
        ...
        // --- METHOD 4: Check data attributes and video containers ---
        if (serverLinks.isEmpty()) {
            debugInfo.add("Method 4: Checking general data attributes")
            val method4Elements = document.select("[data-video]:not([data-video=\"\"]), .play-video [data-video], .video-container [data-video]")
        ...
        // --- METHOD 5: Check all data-value attributes ---
        if (serverLinks.isEmpty()) {
            ...
            document.select("input[type=hidden][value*=/embed/], input[type=hidden][value*=http]").forEach { input ->
        ...
        // Add debug videos
        videos.add(Video(pageUrl, "DEBUG: Processing - ${pageUrl.substringAfterLast("/")}", pageUrl))
        ...
        serverLinks.forEach { (serverName, initialUrl) ->
            ...
            // Handle internal Kajzu embed pages
            if (currentUrl.startsWith(baseUrl, ignoreCase = true) && currentUrl.contains("/embed/", ignoreCase = true)) {
                try {
                    ...
                    val realEmbedUrl = embedDocument.selectFirst("iframe[src], iframe[data-src]")?.attr("src")
                        ?.ifBlank { embedDocument.selectFirst("iframe[data-src]")?.attr("data-src") }
